...
export default function ValueInputBox<T>({propertyRef, identity, range}:ValueInputBox){
   ...
   const description  = () =>{
       ...
       switch (identity){
           case "다양성":{
               text = "무작위성을 가지면서도 일관된 결과를 생성하는 기능으로 높은 값은 출력을 더 무작위로 만들고 낮은 값은 더 집중적이고 결정적으로 만듭니다.";
               break;
           }
           case "일관성":{
               text = "특정 단어나 구문을 사용할 가능성을 낮추는 기능으로 값이 높을수록 모델이 자주 사용하는 단어나 구문을 피하려고 합니다."
               break;
           }
           case "중복성":{
            text = "반복되는 단어나 구문을 피하기 위해 사용되는 기능으로 값이 높을수록 이전 대화에서 사용한 단어나 구문을 더 피하려고 합니다."
               break;
           }
           default :{
               break
    ...
    return (
        <div className={"px-3 py-5 flex-col"}>
            ...
            <span className={"cursor-pointer p-4"} onMouseEnter={()=>descriptionPopupUp()} onMouseOut={()=>descriptionPopupDown()}>{identity} : {value}</span>
            </div>
        </div>
    )
}
